...
namespace
{
	...
	void DrawVertexArray(VertexArray2D& obj)
	{
		...
		for (uint64_t index = 0; index < obj->size(); ++index)
		{
			...
			vertexArray[i + 0].position = sf::Vector2f((float)obj[i].GetVerticePosition<Side::TopLeft>().x, (float)obj[i].GetVerticePosition<Side::TopLeft>().y);
			vertexArray[i + 1].position = sf::Vector2f((float)obj[i].GetVerticePosition<Side::TopRight>().x, (float)obj[i].GetVerticePosition<Side::TopRight>().y);
			vertexArray[i + 2].position = sf::Vector2f((float)obj[i].GetVerticePosition<Side::BottomRight>().x, (float)obj[i].GetVerticePosition<Side::BottomRight>().y);
			vertexArray[i + 3].position = sf::Vector2f((float)obj[i].GetVerticePosition<Side::BottomLeft>().x, (float)obj[i].GetVerticePosition<Side::BottomLeft>().y);
